...
class UserController extends AdminController
{
    ...
    public function createNewUserList($request, $response, $args)
    {
        # 需要一个 userEmail
        ...
        $user->im_type = 2;
        $user->im_value = $email;
        $user->transfer_enable = Tools::toGB(50000);
        $user->invite_num = Config::get('inviteNum');
        $user->auto_reset_day = Config::get('reg_auto_reset_day');
        $user->auto_reset_bandwidth = Config::get('reg_auto_reset_bandwidth');
        $user->money = 0;
        ...
        if((int)$atype == 1 ){
                $user->class_expire = date('Y-m-d H:i:s', time() + Config::get('user_class_expire_default') * 324000);
                $user->class = 1;
                $user->node_group = 1;
                $user->remark = $remark.'unsell';
        }
       ...
       if((int)$atype == 4 ){
        ...
        if ((int)$atype > 7){
        	    $user->class_expire = date('Y-m-d H:i:s', time() + Config::get('user_class_expire_default') * 324000);
                $user->class = (int)$atype;
                $user->node_group = (int)$atype;
                $user->remark = $remark.'unsell';
        }
        ...
        $ssr_sub_token = LinkController::GenerateSSRSubCode($user2->id, 0);
        }
    ...
    public function uploadFile($request, $response, $args){
     ...
     //                var group = new Array();
     //                var class_expire = new Array();
     //                for(var i=0;i<splitdate.length;i++){
     //                     var data =splitdate[i].split(",");
     //                     id.push(data[0]);
     //                     user_name.push(data[1]);
     //                     remark.push(data[2]);
     //                     group.push(data[3]);
     //                     class_expire.push(data[5]);
     //                 }
         $splitdate = $request->getParam('data');
         $splitdate = str_replace('"','', $splitdate);
         $number = count($splitdate);
         //$user = User::where('email', '=', $user_name[0])->first();
          
         for ($x=0; $x<$number-1; $x++) {
         	$user = 33;
         	$data = 66;
            $data = explode(",", $splitdate[$x]);
            $user = User::find($data[0]);
            //$user = User::where('email', '=', $data[0])->first();
            if(is_null($user)){
             $res['ret'] = 1;
             $res['msg'] = json_encode($data);
             $res['email_error'] = 'success';

            }else{
              $user->node_group =(int)$data[3];
              $user->remark = $data[2] ;
              $user->class = (int)$data[3];
              $user->class_expire = date('Y-m-d H:i:s', strtotime($data[5]) );
              //$user->node_group =(int)$data[1];
              //$user->class = (int)$data[1];
              $user->save();

            }

            
          // $user->node_group = $group[$x];
